...
export default function PaymentPage() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!snapToken) {
        throw new Error('Snap token is empty');
      }
      ...
      console.log('🔑 Client Key:', clientKey ? 'Available' : 'MISSING!');
      ...
      if (existingScript) {
        console.log('🗑️ Removing existing Midtrans script');
      ...
      await new Promise((resolve, reject) => {
        ...
        scriptTag.onerror = (err) => {
          console.error('❌ Failed to load Midtrans Snap script:', err);
          reject(new Error('Failed to load Midtrans script - Check your internet connection'));
        };
      ...
      if (!window.snap) {
        console.error('❌ window.snap is not available after script load');
        throw new Error('Midtrans Snap not initialized. Please refresh and try again.');
      }

      console.log('✅ window.snap available, opening payment popup...');
